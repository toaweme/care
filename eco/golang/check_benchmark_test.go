...
goarch: arm64
pkg: github.com/toaweme/care/eco/golang
...
PASS
ok  	github.com/toaweme/care/eco/golang	3.210s
`)
	...
	for _, b := range got {
		if b.Package != "github.com/toaweme/care/eco/golang" {
	...
	if want := []care.BenchMetric{{Unit: "MB/s", Value: 512.34}}; !sameMetrics(tp.Extra, want) {
		t.Fatalf("expected MB/s captured, got %+v", tp.Extra)
	}

	// a ReportMetric custom unit with no standard mem columns.
	custom := got[2]
	if want := []care.BenchMetric{{Unit: "items/op", Value: 3}}; !sameMetrics(custom.Extra, want) {
		t.Fatalf("expected custom items/op captured, got %+v", custom.Extra)
	}
}

func sameMetrics(a, b []care.BenchMetric) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
